# -*- coding: utf-8 -*-
"""
This module is for Testing No.01-001.
...
Attributes:
    ...
    file_r(str)  : filename of input for train
    file_qb(str) : filename of output with predicted values before training
    file_qa(str) : filename of output with predicted values after training
    file_w(str)  : filename of initial weights
    i_from = 1   : Minimum value of inputs
    i_to = 100   : Maximum value of inputs
...
if __name__ == '__main__':
    ...
    PATH = os.path.join(Path(__file__).resolve().parents[1],
                        inifile['GENERAL']['TEST_DIR'])
    file_r = '20181108_01_a_0200-ergebnis.csv'
    file_qb = '20181108_01_a_0200_query_before_#1b.csv'
    file_qa = '20181108_01_a_0200_query_after_#1b.csv'
    file_w = 'weight.csv'
    ...
    csv_weight = pd.read_csv(os.path.join(PATH, file_w), sep=';', header=None)
    nn.weight = csv_weight
    ...
    for index, line in csv_input.iterrows():
        csv_query = csv_query.append(pd.DataFrame(
            data=np.array(nn.query(line), ndmin=2).T), ignore_index=True)
    ...
    result.to_csv(os.path.join(PATH, file_qb), sep=';',
                  index=False, float_format='%.4f',
                  header=['id', 'input1', 'input2', 'output1',
                          'output2', 'predict1', 'predict2'])

    # Train neural network and hold difference between outputs and predictions
    error = pd.DataFrame()
    for _ in range(1):
        for index, line in csv_all.iterrows():
            nn.train((line[['input1', 'input2']] - i_from) /
                     (i_to - i_from), line[['output1', 'output2']])
            error = error.append(pd.DataFrame(
                data=np.array(nn.output_errors).T), ignore_index=True)

    # Show Difference in graph
    ...
    # Add query data in data file (predict1 and predict2)
    result = pd.concat([csv_all, csv_query], axis=1)
    result.to_csv(os.path.join(PATH, file_qa), sep=';',
                  index=False, float_format='%.4f',
                  header=['id', 'input1', 'input2', 'output1',
                          'output2', 'predict1', 'predict2'])
